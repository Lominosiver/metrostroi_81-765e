...
function TRAIN_SYSTEM:Think(dT)
	...
	self.ATS1 = Power and not UOS and Train.SF4.Value > 0.5 and (Train.BARSBlock.Value == 0 or Train.BARSBlock.Value == 1)
	self.ATS2 = Power and not UOS and Train.SF5.Value > 0.5 and (Train.BARSBlock.Value == 0 or Train.BARSBlock.Value == 2)
	...
	if not self.UOS then
		...
		--if Emer then
			...
			if Power and Train.BUKP.State == 5 and KMState > 0 and self.PN1T then self.PN1T = false self.SBTimer = nil end
	...
	if self.PN1 + self.PN2 == 0 and self.StillBrake > 0 then
		self.StillBrake = 0
	end
	--[[
