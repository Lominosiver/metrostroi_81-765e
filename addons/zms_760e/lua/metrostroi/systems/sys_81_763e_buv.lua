...
--------------------------------------------------------------------------------
Metrostroi.DefineSystem("81_763E_BUV")
...
function TRAIN_SYSTEM:Think(dT)
    ...
    local strongerBrake = self.TargetStrength < -2.5 or self.TargetStrength < 0 and self:Get("StrongerBrake") and self:Get("StrongerBrake") > 0
    ...
    if not self.Slope then
        ...
        if self.PTReplaceTimer and CurTime() - self.PTReplaceTimer > 0.5 or (Train.Speed < 7 and self.TargetStrength < -0.9) then
            self.PTReplace = true
