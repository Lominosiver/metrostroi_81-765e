Metrostroi.DefineSystem("81_760E_Prost_Kos")
...
function TRAIN_SYSTEM:Think(dT)
    ...
    if self.Prost then
        ...
        if self.dist2 < -10 then self.dist2 = 350 end
        ...
        if Train.KV765.Position >= 0 then
        ...
        if Programm then
            ...
            local find = false
            for k, v in pairs(self.Metka) do
                if Programm == v then find = true end
            end

            if not find then self.Metka[self.Metk] = Programm end
            if self.Metka[1] == Programm then self.RightDoors = Programm.RightDoors or false end
            if Train.KV765.Position >= 0 and not self.Metka[3] and self.Metka[2] == Programm and not self.Metka[1] or self.Metk > 2 then self.ProstActive = 1 end
        ...
        if dist2 < 350 and self.prevdist3 and dist3 - 1 > self.prevdist3 then self.WrongDir = true end
        if dist2 >= 350 and self.WrongDir then self.WrongDir = false end
        if dist2 < 200 and Train.KV765.Position < 0 and (self.Metka[3] or self.Metka[2]) then self.ProstActive = 0 end
        ...
        if RV > 0 and power and Train.BUKP.State == 5 and Train:GetNW2Int("VityazMainMsg", 0) == 0 and not Train.BKL then
            ...
            if Programm and command == 2 and not self.Metka[2] then self.Metka[2] = Programm end
    ...
    if self.Kos then --and self.ProstActive == 0 then
        ...
        if Programm then
            if self.Metka[2] == Programm and Tspeed > 40 then self.CommandKos = true end
            if self.Metka[1] == Programm and Tspeed > 20 then self.CommandKos = true end
        end

        if dist1 < 0 and Tspeed < 1 and not self.Stop then self.Stop = true end
        ...
        if not self.Metka[1] and not self.Metka[2] and not self.Metka[3] and not self.Metka[4] and not self.PrKos then self.PrKos = true end
        if not self.Metka[2] and not self.Metka[1] and self.Stop then
            --self.CommandKos = false
            self.Stop = false
        end

        if self.Metka[1] and Tspeed < 1 and math.abs(dist2) < 3 and not self.Stop1 then self.Stop1 = true end
        if not self.Metka[2] and not self.Metka[1] and self.Stop1 then self.Stop1 = false end
        if not self.Metka[2] and not self.Metka[1] and self.WrongPath then self.WrongPath = false end
        if (RV == 0 or Train.KV765.Position < 0) and self.CommandKos then
            self.CommandKos = false
            self.KosTimer = nil
            self.EmergencyBrake = 0
            self.Stop = false
        ...
        if self.CommandKos and not self.KosTimer then self.KosTimer = CurTime() + 1 end
