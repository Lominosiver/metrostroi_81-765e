...
function TRAIN_SYSTEM:Think(dT)
	...
	if self.Prost then 
		...
		if Programm then
			...
			if Train.KV765.Position >= 0 and not self.Metka[3] and self.Metka[2] == Programm and not self.Metka[1] or self.Metk > 2 then
				self.ProstActive = 1
			end
		...
		if dist2 < 200 and Train.KV765.Position < 0 and (self.Metka[3] or self.Metka[2]) then
			self.ProstActive = 0
		end
