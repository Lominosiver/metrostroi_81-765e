...
function TRAIN_SYSTEM:Think(dT)
    ...
    local SchemeWork = (Train:ReadTrainWire(6) * Train.SF50.Value > 0.5 or (Train:ReadTrainWire(
        --[[Train:ReadTrainWire(5)*Train.SF51.Value > 0.5 or]]
        14) + Train:ReadTrainWire(15) == 1 and 1 or 0) * Train.SF51.Value > 0) and 1 or 0
    ...
    local bv_off = (self:Get(
        --[[ self.TimerCount >= 3 and (not self:Get("KRReserve1") and Train:ReadTrainWire(19)+Train:ReadTrainWire(45) == 0) or]]
        "BVOff") or Train.Battery.Value * Train.SF52.Value == 0 or Train.Electric.Battery80V <= 67) and (Train:ReadTrainWire(19) + Train:ReadTrainWire(45) == 0) or self:Get("PVU1")
    ...
    local strongerBrake = self.TargetStrength < 0 and self:Get("StrongerBrake") and self:Get("StrongerBrake") > 0
    --[[
